...
namespace VKE
{
	...
	void cEmitter::init(FMainDevice* const iMainDevice)
	{
		...
		// Create particle count storage buffer
		{
			// Create storage buffer
			VkDeviceSize StorageBufferSize = sizeof(BufferFormats::FParticleCount);
			cBuffer StagingBuffer;
			...
			initialParticleCount.ParticleCount = 0;
			// Map particle data to the staging buffer
			void * pData = nullptr;
			vkMapMemory(iMainDevice->LD, StagingBuffer.GetMemory(), 0, StorageBufferSize, 0, &pData);
			memcpy(pData, &initialParticleCount, static_cast<size_t>(StorageBufferSize));
			vkUnmapMemory(iMainDevice->LD, StagingBuffer.GetMemory());

			// Create storage buffer, Binding = 3
			ComputeDescriptorSet.CreateStorageBufferDescriptor(StorageBufferSize, 1, VK_SHADER_STAGE_COMPUTE_BIT,
				// 1. As transfer destination from staging buffer, 2. As storage buffer storing particle data in compute shader
			...
			// Command to copy src buffer to dst buffer
			const cBuffer& StorageBuffer = ComputeDescriptorSet.GetDescriptorAt<cDescriptor_Buffer>(3)->GetBuffer();
			vkCmdCopyBuffer(TransferCommandBuffer, StagingBuffer.GetvkBuffer(), StorageBuffer.GetvkBuffer(), 1, &BufferCopyRegion);
			// Setup a barrier when compute queue is not the same as the graphic queue
			if (iMainDevice->NeedSynchronization())
			{
	...
	void cEmitter::NextParticle(BufferFormats::FParticle& oParticle, int idx)
	{
		// Position
		...
		oPos.x += R * cosTheta;
		oPos.z += R * sinTheta;
		// oParticle is in world space
		oParticle.Pos = glm::vec3(EmitterData.ModelMatrix * glm::vec4(oPos, 1.0));
		...
		oParticle.ElpasedTime = -static_cast<float>(idx) / ParticleSupportData.EmitRateOverTime;
	...
	void cEmitter::UpdateEmitterData()
	{
		if (!bNeedUpdate)
		{
			return;
		...
		EmitterData.ModelMatrix = Transform.M();
		ComputeDescriptorSet.GetDescriptorAt<cDescriptor_Buffer>(2)->UpdateBufferData(&EmitterData);
		bNeedUpdate = false;
	}
	...
	void cEmitter::MoveVertically(float axis)
	{
		if (!IsFloatZero(axis))
		{
			Transform.Translate(axis * (float)Time::DT * Transform.Right());
			Transform.Update();
			bNeedUpdate = true;
			UpdateEmitterData();
		}
